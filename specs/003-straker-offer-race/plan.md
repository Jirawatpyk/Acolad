...
| **Two daily ceilings, where FR-009 specifies one** — `STRAKER_MAX_WORDS_PER_DAY` for translation and `STRAKER_DTP_MAX_WORDS_PER_DAY` for monolingual (DTP) work, each with its own throughput derived from it. Added 2026-09-17 after the portal offered DTP work the model did not contain. **Live at 3,500 and 30,000.** | FR-009 says effort is the raw word count, and it still is — but a word of DTP preparation and a word of translation are counted in the same unit and are not the same commitment. One ceiling over both means a morning of formatting refuses an afternoon of translation, or the reverse, depending only on which arrived first. The ledger therefore keys every total by kind (`ledger.ts`), and the combined 09:00 view labels the two parts rather than adding them, which is the remedy FR-018 itself offers for unlike quantities. | Refusing DTP work outright was the previous behaviour and was rejected by the owner: the portal sends it, the team does it. Claiming DTP against the translation ceiling was rejected because a single 956-word DTP job would consume a quarter of a day's translation budget it was never measured against. **The honest weakness is the number, not the mechanism**: nobody has measured how fast the team does DTP. Exactly one DTP job has ever been seen (956 words, 2026-09-17), and `config.ts` says in as many words that it "might be an hour of formatting or four" — at four hours that is ~239 w/h, *slower* than translation's 389, and a 30,000-word day would be committing something closer to a fortnight of work to one deadline, irreversibly. 30,000 is the owner's figure, set knowing this, and 30,000 ÷ 9 working hours = 3,333 w/h is what the gate then measures feasibility at — one knob, not two (`STRAKER_DTP_THROUGHPUT_WORDS_PER_HOUR` stays blank deliberately; pinning it is how a ceiling and its rate drift apart). **Withdraw or lower this on any of**: the first measured DTP completion showing a rate below ~1,000 w/h; any day where DTP holdings exceed one working day's plausible output; or RP-4 closing, at which point the figure must be replaced by an observed one rather than inherited. Until then the combined 09:00 view is the only thing showing the total, which is visibility rather than enforcement — the same trade the row below records for the two portals. |
| **Tracking-sheet timestamps render `DD/MM/YYYY HH:mm` instead of ISO 8601** (Constitution III: "All user-facing timestamps MUST use ISO 8601 in Asia/Bangkok"). Added 2026-09-17 at the owner's request that the sheet be readable. | The clause's own rationale is readability — operators must "scan, filter, and trust them without decoding format drift". The operator reads Straker's sheet and the XTM sheet side by side, and the XTM sheet has rendered `DD/MM/YYYY HH:mm` since 002; **two formats across those two sheets is itself the drift the clause exists to prevent**. Honouring the letter here defeated the purpose, so the letter gives way and the deviation is recorded. | Keeping ISO in this sheet alone was rejected for the reason above. Re-pointing the XTM sheet at ISO instead was rejected as far worse: it rewrites the format of a record that has been read daily since June, for a clause whose point it would not serve. **Seconds were the real cost and were not paid**: SC-001's latency measure lives in First seen and Claimed at, races are decided inside two seconds, and minute resolution would quantise the measure to zero — so those two columns keep `:ss` and Deadline, which never had a second hand, does not. **A second cost, named late for honesty:** the cells are written with `valueInputOption: 'RAW'`, so `DD/MM/YYYY` is text and no longer sorts or filters chronologically in Sheets — `01/10/2026` sorts above `17/09/2026`, where ISO sorted correctly for free. The XTM sheet has always had this property, so it does not reverse the consistency argument, but the row above claimed completeness about the costs and this one was missing. **Withdraw this row if** a third consumer needs to machine-parse the sheet, or if sorting by date becomes something anyone does — both are better served by a real date-typed cell than by either text format. |
