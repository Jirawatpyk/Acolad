...
- [x] T076 Report the Straker win rate automatically instead of only on demand, per SC-004 (partial) — `computeWinRate` has exactly one caller, `winRateReport.ts`, which is the manual `npm run straker:win-rate`. The 09:00 combined report already carries workload, retries and uptime, so it is the obvious home. The consequence of leaving it is not a missing number but a missing *habit*: SC-004 sets a target only after two weeks of baseline, and a baseline nobody is shown is a baseline nobody reads. Note the report is in the XTM bot's path (`dailyReport.ts`), so this must go through `combinedSummary.ts` like FR-018 did, and must keep `combinedReportRows`'s never-throws property. **DONE.** `strakerWinRateRow` in `combinedSummary.ts`, appended by `combinedReportRows` so it rides the existing 09:00 report. Deliberately NOT part of the combined view: that machinery decides when two portals' numbers may be added, and a win rate is not combinable — XTM has no equivalent and averaging one against nothing would invent a figure. It refuses three ways of misleading: no bare percentage (the counts travel with it), never `0%` when nothing was winnable (FR-017 excludes only work our own rules turned away), and never silence on an unreadable record. `WEAK_SIGNAL_BELOW` moved to `winRate.ts` so the script and the report cannot disagree about what counts as a weak signal. Seam mutation-checked: removing the wiring fails the test.
- [x] T077 Guard FR-030 (DC-4) with a walking test the way R11 is guarded, per FR-030 (partial) — the invariant currently HOLDS: no `fetch(` exists in `src/straker/` outside `httpClient.ts`. Nothing enforces it. R11's equivalent is enforced at `tests/integration/straker/isolation.test.ts:652`, and `httpClient.ts` names the exact failure this would catch — a *racing* read added through `getJson` silently joins the deferrable class and gets shed below 120 remaining, which is the hot path quietly losing its budget priority. Walk `src/straker/**` and fail on any request-issuing call outside the one file; assert the guard is not vacuous (a deliberate violation must fail it). **DONE.** Walking guard in `isolation.test.ts`, mirroring R11's. Two things the guard-the-guard tests caught while being written, both of which would have made it pass vacuously: `httpClient.ts` never writes `fetch(` — it writes `options.fetchImpl ?? fetch`, so a call-site detector found nothing at all; and `pollCycle.ts` logs `action: 'fetch'`, a string literal the detector counted as wire access, whose obvious "fix" would have been to exempt the very file the rule most needs to cover. The rule is therefore "a bare `fetch` in a VALUE position", with `typeof fetch` excluded for the same reason R11 excludes `import type`, and `main.ts`'s one type-only reference pinned by name. Verified non-vacuous by planting a real `fetch(` in `eligibility.ts`: the guard fails and names the file.
- [x] T078 Decide whether the probe's stranded core stays or goes, per T072/RP-5 (unrequested) — `CaptureStore` (`src/straker/captureStore.ts`) and `runProbeCycle` (`src/straker/probe.ts`) have had no production caller since T072 removed the probe entry point; only their tests reach them. RP-5 recorded keeping them deliberately, because "Straker's payload shape is confirmed on two jobs and resurrecting evidence collection should be cheap". That reasoning may well still hold — this task is to make it a decision with a date rather than drift, and to record the outcome either way. `probe.ts` itself must stay regardless: `RawOffer` is used throughout the bot. **DONE — decided KEEP, 2026-09-16**, recorded in `release-preconditions.md` under RP-5 with a reopen trigger. The deciding fact had not been written down anywhere: **the live bot does not preserve raw offer payloads** — it parses, and fails loud on a shape it does not recognise (FR-023), but the payload that would let someone fix the parser is kept nowhere. `CaptureStore` is the only code that writes one to disk, while `offerParse.ts` still opens by saying the evidence is two independent jobs and RP-4 has put no real claim through the path. Reopen: delete once the parser is confirmed on ten or more independent offers, or a month after RP-4 closes with shapes holding steady.
- [x] T079 Bring `plan.md`'s Source Code block in line with the tree, per plan: Source Code structure (contradicts) — it still lists `reconMain.ts`, which T072 deleted, and omits four files that shipped since: `dispatcher.ts` (Phase 4), `requeue.ts` and `winRate.ts` (review-fix waves), `unbar.ts` (T073). Documentation-only; no code changes. **DONE.** `plan.md` §Source Code now matches the tree: `reconMain.ts` marked DELETED by T072, the five "not built" files marked built, and the four the plan never named added — `dispatcher.ts`, `winRate.ts`, `requeue.ts`, `unbar.ts`. Also corrected the STATUS header (was "as of 2026-09-15, Phases 1-3") and `tests/live/straker/`, which the block still described as not created.
