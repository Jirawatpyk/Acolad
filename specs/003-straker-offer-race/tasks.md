...
- [x] T040 [US1] Define the skip reasons in `src/straker/types.ts` and record one for every offer not claimed in `src/straker/pollCycle.ts`, including `exceeds_daily_ceiling_entirely` as distinct from an ordinary ceiling skip (FR-010, data-model §4)
