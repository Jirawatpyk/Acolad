...
- **The deadline falls on a weekend, a Thai public holiday, or in a year whose holiday calendar has not been curated** — same governance as XTM; an uncurated year must block claiming rather than permit it.
  - **Amended 2026-09-18 (owner decision):** a deadline that falls on a weekend or holiday is **no longer a refusal on its own**. It is judged like any other deadline — the work must fit in the *working* time before it, which a day off does not add to. Two 43-word offers seen at 02:42 on Friday 18/09 were refused for being due Saturday 12:59 with nine working hours still left that day, and the owner's ruling was that the weekday is not the question, the time is. An uncurated year still blocks. The XTM bot keeps the original rule; the shared gate takes an opt-in flag that only Straker sets.
