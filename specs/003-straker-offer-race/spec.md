...
- **FR-014**: The system MUST record every offer it sees, with its outcome and reason, in a tracking record **of its own** — a separate file from the XTM record, not a section within it.
...
- **FR-030 (DC-4)**: All transport-level concerns for Straker MUST live in a single place, so that a future third portal of the same family starts by copying one file.
