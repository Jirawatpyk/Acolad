...
- **The deadline falls on a weekend, a Thai public holiday, or in a year whose holiday calendar has not been curated** — same governance as XTM; an uncurated year must block claiming rather than permit it.
  - **Amended 2026-09-18 (owner decision):** a deadline that falls on a weekend or holiday is **no longer a refusal on its own**. It is judged like any other deadline — the work must fit in the *working* time before it, which a day off does not add to. Two 43-word offers seen at 02:42 on Friday 18/09 were refused for being due Saturday 12:59 with nine working hours still left that day, and the owner's ruling was that the weekday is not the question, the time is. An uncurated year still blocks. Adopted the same day as the shared scheduling standard for the XTM bot too, with no opt-in flag: any bot using the shared gate gets it.
...
- **FR-008**: The system MUST apply the team's existing scheduling rules — working hours, working days, Thai public holidays, deadline reachability at the team's throughput, and a daily workload ceiling — before claiming any offer.
  - **Amended 2026-09-18 (owner decision) — how the daily ceiling is spent.** The ceiling is a per-*working-day* figure and a deadline has the working time before it: an offer fits when, for every deadline day *d* from its own onward, the work due on or before *d* fits in what the working time left through *d* can hold — the ceiling pro-rated over those working minutes, never less than one day's ceiling (earliest-deadline-first). It had been charged whole to its deadline day alone, so a 4,000-word job due Wednesday was permanently unclaimable on Monday at 3,500 a day, though the three days before it hold 10,500; the owner ruled such a job must be claimable. A first cut counted whole *days*, and review found it admitted 6,800 words at 17:45 into ~3,600 words of real time — hence working time left, not days touched. The rule is never looser than the per-deadline-day one for a deadline under a day away, and can be stricter by design: it counts overdue work, earlier work, and later commitments a new job would crowd out. The XTM bot adopted the same rule the same day (`schedule/windowCapacity`, shared by both).
...
- **FR-018**: The daily summary MUST present the committed workload of **both** portals, each separately and combined, together with **retries performed and uptime** for the period (named by the constitution's observability principle, and the two figures that reveal a bot limping rather than failing), and MUST produce this view without participating in any claim decision. Because the two records now live in separate files, the summary MUST read both and MUST state plainly when either is unreadable rather than presenting a partial total as if it were whole. The combined total MUST be shown only while **both portals measure effort in the same unit**; if either ever switches, the total MUST be suppressed or labelled rather than adding unlike quantities.
  - **Amended 2026-09-22 (owner decision).** The daily summary is split by room. **Straker sends its own 09:00 report** into its own announcement channel on working days — due today per budget (translation and DTP separately, by effective deadline day, against each ceiling), overdue work, the five nearest deadlines by file · job ref · service, and the 14-day win rate with its turn-away count (FR-017/017a) — and sends **nothing** when nothing is held and the win rate has nothing to say. The **XTM 09:00 report keeps only its own figure and one combined line** (`Both portals`, naming each portal's part), still suppressed/labelled under the unreadable and unlike-units rules above; it is sent only when XTM holds work or that line is a warning, never for Straker work alone. The win rate counts each piece of work **once**: a recovery recorded under a purchase-order or assigned-job id is merged into the claim it settles. Retries and uptime remain in `npm run report:combined`. Contract: `contracts/straker-reporting.md` §5a/§5b.
...
- **FR-020** *(DEFERRED 2026-09-15 — see T044; not met, and deliberately so)*: The system MUST keep its connection to the portal warm between checks rather than establishing a new one each time.
  - **Status: not implemented.** The transport runs on the platform `fetch` with no pooling agent and no `undici` dependency. This is the strike rule's conclusion applied honestly rather than quietly: the rule formally KEEPS SC-001/SC-002 because the sample is under 10, yet the measurement it rests on says the race is decided at **204 seconds**, not at the 400 ms SC-001 bounds. Pooling exists to win a race decided in milliseconds.
  - **What would reopen it**: a single observation of a materially shorter offer lifetime — not a larger count of long ones. Marked here rather than left reading as satisfied, because a requirement that quietly is not met is the worse of the three options (the principle T074 named).
- **FR-021** *(STRUCK 2026-09-16 on the T044 evidence — closes T074)*: ~~The system MUST renew its session **before** it lapses, rather than waiting for the portal to reject a request.~~
  - **Replaced by**: the system MUST re-establish its session **on rejection**, and a re-sign-in MUST NOT cost an offer. This is R5's single reactive re-sign-in, which is what `session.ts` implements (`session ??= await signIn()`).
  - **Why struck rather than implemented**: proactive renewal buys time measured in milliseconds against a race measured at **204 seconds** — the same reasoning that deferred FR-020, which T044 recorded but never connected back to this requirement. Reactive renewal costs at most one read, and the reconciliation pass (FR-016a) closes behind anything the gap dropped.
  - **What would reopen it**: evidence that a lapsed session costs a *claim* rather than a read.
...
- **SC-007** *(RP-4 closed 2026-09-22 — the lost-race signal is HTTP 409, confirmed from the portal's web app; 17 live wins through the same claim path)*: A lost race **never** produces an operational alert and **never** halts the loop, demonstrated by a failure-mode test.
  - **Met today**: "never halts the loop" holds unconditionally, and the no-alert path is demonstrated by test.
  - **NOT met today**: `CONFIRMED_LOST_RACE_SIGNALS` in `claimOutcome.ts` is **deliberately empty** until RP-4 confirms a real signal, so `classifyClaim` can recognise no actual loss — every rejection falls through to `failed` and alerts. The demonstrating test therefore passes an injected list (`SIGNALS_AS_IF_CONFIRMED`), not the production constant, which is why the test is green while the criterion is not.
  - This is the safe direction (FR-005a): an unrecognised rejection is never *inferred* to be an ordinary loss. It is also loud — in production the commonest non-win outcome pages someone every time.
  - **Closes when** RP-4 adds one value to that array. Marked conditional for the same reason SC-001/SC-002 are: a criterion that reads as satisfied while its production path cannot satisfy it is worse than one that states its condition.
...
- **SC-010**: Stopping either bot while the other keeps running raises an alert naming the stopped one **within 10 minutes**. The figure is stated here rather than by reference to another system's configuration, which could change without anyone noticing this criterion moved with it.
  - **This criterion, and Constitution IV's heartbeat MUST with it, rests on state that is NOT in this repository.** Each bot's dead-man switch is a healthchecks.io check, and its period, its grace, and — the part that actually decides whether anyone is paged — **which notification integrations are attached to it** are account settings, not version-controlled configuration.
  - A newly created check accepts pings and satisfies every test here while notifying nobody, because integrations are selected per check rather than inherited. Verifying SC-010 therefore means opening the check, not reading the code.
  - **Required settings per check**: period **60s**, grace **300s** (5 min, inside the 10-minute bound), Google Chat integration **enabled**. See `quickstart.md` V-HC.
