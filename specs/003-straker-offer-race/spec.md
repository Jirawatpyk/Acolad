...
- **FR-008**: The system MUST apply the team's existing scheduling rules — working hours, working days, Thai public holidays, deadline reachability at the team's throughput, and a daily workload ceiling — before claiming any offer.
  - **Amended 2026-09-18 (owner decision) — how the daily ceiling is spent.** The ceiling is a per-*working-day* figure and a deadline has every working day before it: an offer fits when, for every deadline day *d* from its own onward, the work due on or before *d* fits in `ceiling × working days from today through d` (earliest-deadline-first). It had been charged whole to its deadline day alone, so a 4,000-word job due Wednesday was permanently unclaimable on Monday at 3,500 a day, though the three days before it hold 10,500; the owner ruled such a job must be claimable. For work due today the two rules are identical. The XTM bot keeps the per-deadline-day rule.
