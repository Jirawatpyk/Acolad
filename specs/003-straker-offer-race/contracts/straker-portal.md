...
| `due_at` is zone-less, and **may carry a six-digit fraction** | Zone-less: confirmed. The fraction was **observed live 2026-09-25** — six offers carrying `2026-09-28T00:05:50.987723`, every one refused as unreadable until the parser was widened. Such deadlines are machine-computed (creation + 72 h); a human-set one lands on a round `08:00:00`, which is why the captured payloads never showed it. The parser now accepts the fraction and truncates to milliseconds, and **still rejects any zone designator** — precision says nothing about zone, so widening one does not concede the other. The assigned-jobs reader has accepted the same six digits since it was written. |
