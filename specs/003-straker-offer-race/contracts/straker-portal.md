...
- **On start, one reconciliation pass runs before the first poll cycle**, and the poll cycle never claims an offer whose key matches held work — so a claim whose POST landed but whose record died with the process cannot be sent twice after a restart (FR-019c). **No claim is dispatched at all until one pass has completed successfully** since start (a shed or failed pass does not count); until then the bot reads, records and notifies as usual and logs `action:claim outcome:held_until_reconciled` once per cycle. A reconciler that keeps failing is surfaced by `reconcile_failing` (three consecutive failures).
