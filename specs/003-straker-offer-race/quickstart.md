...
| **V-HC** | **Each bot's healthchecks.io check has period 60s, grace 300s, and the Google Chat integration ENABLED** | **M** | SC-010, Constitution IV | **Verified by opening the check, not by running anything.** The ping URL is the only part of the dead-man switch that lives in `.env`; period, grace and integrations are account settings. A new check accepts pings and passes every automated test here while notifying nobody, because integrations are selected per check and are not inherited from an existing one. Re-verify whenever a check is created or recreated — the Straker check was created 2026-09-16 |
