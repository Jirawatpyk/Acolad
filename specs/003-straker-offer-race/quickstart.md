...
Legend: **A** = buildable now · **B** = needed real offer payloads; **that wait ended on 2026-09-15 by decision rather than by V19 being met** (3 offers, not 10 — see V19) · **P** = the probe itself · **M** = **manual** — verified by a human opening an external system, because the state being checked is not in this repository and no test here can see it
...
| **V-HC** | **Each bot's healthchecks.io check has period 60s, grace 300s, the Google Chat integration ENABLED, and a name that says which bot it is** | **M** | SC-010, Constitution IV | **✅ BOTH checks verified 2026-09-16.** Verified by opening them, not by running anything: the ping URL is the only part of the dead-man switch that lives in `.env`, while period, grace, integrations and the check's NAME are account settings no test here can see. **Found already correct**: Google Chat ON on both, with an email fallback — so the failure this row exists to catch, a check that accepts pings and notifies nobody, was not present. **Corrected on both**: Period was **5 minutes**, which with the 5-minute grace put detection at exactly **10 minutes** — on SC-010's bound rather than inside it. Both now **1 minute**, so detection is **~6 minutes**. **Corrected on both**: the names were `NZTC` and `My First Check` (the healthchecks default, never changed since the XTM bot was first set up). Now `jobcatch-straker` and `acolad-bot (XTM)`. This matters because SC-010 requires the alert to *name the stopped one* and the notification carries the check name — `My First Check is DOWN` at 2am identifies nothing. **Evidence that 1 minute is safe**, measured over 7 days of XTM logs: 26,558 cycles, worst gap between cycles **51s**, zero gaps over 60s. The margin is far wider than that comparison suggests, because Period alone never alerts — a ping slower than Period only marks the check *late*, and it pages only after `Period + Grace`, so the real threshold is **6 minutes of total silence** against a worst observed gap of 51s. Re-verify the whole row whenever a check is created or recreated |
