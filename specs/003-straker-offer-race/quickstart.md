...
| **V-HC** | **Each bot's healthchecks.io check has period 60s, grace 300s, and the Google Chat integration ENABLED** | **M** | SC-010, Constitution IV | **✅ Straker check verified 2026-09-16.** Verified by opening the check, not by running anything: the ping URL is the only part of the dead-man switch that lives in `.env`, while period, grace and integrations are account settings no test here can see. **Found**: Google Chat **ON** (plus an email fallback) — so the dangerous half, a check that accepts pings and notifies nobody, was not present. **Corrected**: Period was **5 minutes**, which with the 5-minute grace put detection at exactly **10 minutes** — *on* SC-010's bound rather than inside it. Set to **1 minute**, so detection is now **~6 minutes**. Period is "the slowest gap still acceptable", not "how often to ping", so lowering it raises no false alarms against a bot that pings every cycle. **Not verified**: that the check's NAME distinguishes Straker from XTM — SC-010 requires the alert to *name the stopped one*, and the notification carries the check name. Re-verify the whole row whenever a check is created or recreated |
