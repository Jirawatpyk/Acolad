...
- [ ] T013 Job store ใน `src/state/jobStore.ts`: upsert จาก snapshot,
      state transitions ตาม data-model (missing ≥ 2 รอบ), อัปเดต
      snapshot_hash + log การเปลี่ยนรายละเอียดแบบเงียบ (FR-019), meta
      helpers (baseline_done, login_failure_count, lockout_until,
...
- [ ] T019 [P] [US1] สร้าง fixtures HTML ใน `tests/fixtures/`: หน้า login,
      ...
      ไม่มี marker / มีตัวบ่งชี้ pagination) + mock Google Chat webhook
      server (undici MockAgent) ใน `tests/fixtures/mocks.ts`
...
- [ ] T028 [US1] Dispatcher ใน `src/reporting/dispatcher.ts`: flush outbox
      ทุก 5 วิ, backoff ต่อรายการ, dead → สร้าง system event, รวบเป็นชุด
      เมื่อ > 10 งาน/รอบ (SC-008), ความล้มเหลวไม่บล็อก detection
      (Constitution IV)
- [ ] T029 [US1] Poll cycle + loop ใน `src/runtime/pollLoop.ts`: 1 รอบ =
      ensureLoggedIn → fetchSnapshot → diff → persist+enqueue ใน txn เดียว
      → heartbeat hook; interval 25s ± jitter clamp [20s,30s] (FR-003/FR-011),
      ตัวนับ requests/ชม. ≤ 180
- [ ] T030 [US1] Cold start ใน pollLoop/jobStore: baseline ครั้งแรก →
      system event `cold_start_summary` 1 รายการ (รวมกรณี 0 งาน — FR-015),
      ตรวจ cold start ซ้ำใน 7 วันจาก log → alert warn
...
- [ ] T032 [US1] Integration tests ใน `tests/integration/detect.test.ts`:
      ...
      ไม่ซ้ำ), V3 (25 งานครบ), V11 (cold start 0/3/25 + ไม่ซ้ำรอบถัดไป)
...
- [ ] T035 [US2] Error backoff ใน `src/runtime/pollLoop.ts`: exponential +
      ...
      ส่งสัญญาณ alert (ผูกกับ US3 trigger), ฟื้นแล้วกลับ interval ปกติ
- [ ] T036 [US2] Heartbeat ใน `src/monitoring/heartbeat.ts`: ping
      HEALTHCHECKS_PING_URL หลังรอบสำเร็จ (≤ 5 นาที/ครั้ง — FR-010),
      ping `/fail` ระหว่าง lockout/backoff ยาว/outbox dead (notifications
      contract), ping ห้าม throw กลับเข้า loop
...
- [ ] T042 [US3] System alert flow ใน `src/reporting/notifier.ts` +
      dispatcher: render SYSTEM_ALERT ตามตาราง action 8 trigger
      (notifications contract) + SYSTEM_RECOVERED เมื่อ resolve — ส่งผ่าน
      outbox เดียวกัน (ห้ามส่งตรง)
- [ ] T043 [US3] Wiring ครบทุก trigger ใน `src/runtime/pollLoop.ts` +
      dispatcher: portal ล่ม > 10 นาที, outbox dead → alert + `/fail`,
      cold start ซ้ำ, db corrupt (จาก T012) — แต่ละ trigger จับคู่
      dedup_key + recovered
...
- [ ] T048 Final gates: `npm run lint` + `npm run typecheck` +
      ...
      quickstart บันทึกผล + ตรวจ memory < 1 GB ระหว่างรันยาว
...
- V14 (30 วัน) เริ่มนับหลัง deploy จริง — ไม่บล็อกการปิด tasks
