...
- [x] T001 Initialize Node 22 + TypeScript project: `package.json` พร้อม
...
- [x] T002 [P] `tsconfig.json` แบบ strict ทั้งหมด (strict, noUncheckedIndexedAccess,
      exactOptionalPropertyTypes) target ES2022 module NodeNext
- [x] T003 [P] ตั้งค่า ESLint + Prettier: `eslint.config.js`, `.prettierrc`
      — `npm run lint` ต้อง 0 error บนโครงว่าง
- [x] T004 [P] `vitest.config.ts`: coverage v8 + gate ≥ 80% lines เฉพาะ
      `src/detection/`, `src/state/`, `src/reporting/` (Constitution II)
- [x] T005 สร้างโครงโฟลเดอร์ตาม plan.md: `src/{config,portal,detection,state,reporting,monitoring,runtime}/`,
      `tests/{unit,integration,fixtures}/` พร้อมไฟล์ `.gitkeep`
- [x] T006 [P] `ecosystem.config.cjs` สำหรับ PM2: ชื่อ app `acolad-bot`,
      `max_memory_restart: '900M'`, log paths ใต้ `logs/`, autorestart
- [x] T007 ติดตั้ง browser: `npx playwright install chromium` และยืนยันรันได้
- [x] T050 [P] ตั้ง GitHub Actions CI ใน `.github/workflows/ci.yml`: รัน
...
- [x] T011 [P] Unit tests (เขียนก่อน ต้อง FAIL): state stores ใน
...
- [x] T012 DB layer ใน `src/state/db.ts`: better-sqlite3 WAL + migration
...
- [x] T013 Job store ใน `src/state/jobStore.ts`: persist ผลลัพธ์ transition
...
- [x] T014 [P] Outbox store ใน `src/state/outbox.ts`: enqueue ใน transaction
...
- [x] T015 [P] System events store ใน `src/state/systemEvents.ts`: สร้าง
...
- [x] T016 [P] [US1] Unit tests jobKey ใน `tests/unit/jobKey.test.ts`:
      ใช้ portal id เมื่อมี, fallback hash ตามกติกา serialize null
      (FR-005/data-model) — ค่าเดิม → key เดิมเสมอ (deterministic)
- [x] T017 [P] [US1] Unit tests diff ใน `tests/unit/diff.test.ts`:
...
- [x] T018 [P] [US1] Unit tests formatter ใน `tests/unit/notifier.test.ts`:
      ...
      "…และอีก 5 งาน")
- [x] T019 [P] [US1] สร้าง fixtures HTML ใน `tests/fixtures/`: หน้า login,
...
- [x] T020 [US1] jobKey ใน `src/detection/jobKey.ts` (pure function ตาม T016)
- [x] T021 [US1] diff ใน `src/detection/diff.ts` (pure: snapshot + สถานะเดิม
      รวม `consecutive_misses` → รายการ events + สถานะใหม่ — **เจ้าของ
      กติกา transition ทั้งหมด** ตาม data-model; ไม่มี I/O)
- [x] T022 [US1] Selector registry ใน `src/portal/selectors.ts`: รวมศูนย์
      selector ทั้งหมด (ฟอร์ม login, container รายการงาน = empty marker,
      แถวงาน + ฟิลด์, ตัวบ่งชี้ pagination, ตัวบ่งชี้ CAPTCHA) — R9
- [x] T023 [US1] Browser lifecycle ใน `src/portal/browser.ts`: Chromium
...
- [x] T024 [US1] Login flow ใน `src/portal/login.ts`: FR-001 ด้วย selectors
      registry, ตรวจ CAPTCHA/2FA → `CaptchaDetectedError` (ห้าม retry),
      login flow timeout 60s ตาม portal-adapter contract
- [x] T025 [US1] Job list reader ใน `src/portal/jobList.ts`: refresh 1 ครั้ง/
...
- [x] T026 [US1] Formatter ใน `src/reporting/notifier.ts`: interface
      Notifier + render template 5 ประเภท (ตาม T018)
- [x] T027 [P] [US1] Google Chat adapter ใน `src/reporting/googleChat.ts`:
      POST `{text}` UTF-8, จำแนกผลตอบ 429/5xx/network = ชั่วคราว vs
      401/403/404 = ถาวร (ตารางใน notifications contract)
- [x] T028 [US1] Dispatcher ใน `src/reporting/dispatcher.ts`: flush outbox
...
- [x] T029 [US1] Poll cycle + loop ใน `src/runtime/pollLoop.ts`: 1 รอบ =
...
- [x] T030 [US1] Cold start ใน `src/runtime/pollLoop.ts` (จุดทำ baseline):
      ...
      → alert warn
- [x] T031 [US1] Entrypoints: `src/runtime/once.ts` (รอบเดียวจบ สำหรับ
      `poll:once`) + `src/runtime/main.ts` (loop ถาวร + graceful shutdown
      ปิด browser/db สะอาด)
- [x] T032 [US1] Integration tests ใน `tests/integration/detect.test.ts`:
...
- [x] T033 [P] [US2] Failure-mode tests ใน `tests/integration/recovery.test.ts`:
...
- [x] T034 [US2] Session expiry detection ใน `src/portal/jobList.ts` +
      `login.ts`: เด้งหน้า login กลางคัน → `SessionExpiredError` →
      ensureLoggedIn แล้ว retry 1 ครั้งในรอบเดิม (portal-adapter contract)
- [x] T035 [US2] Error backoff ใน `src/runtime/pollLoop.ts`: exponential +
...
- [x] T036 [US2] Heartbeat ใน `src/monitoring/heartbeat.ts`: ping
...
- [x] T037 [US2] Browser recycle ใน `src/portal/browser.ts` + pollLoop:
      ทุก BROWSER_RECYCLE_HOURS เปิดใหม่ก่อนปิดเก่า — เสร็จใน 1 รอบ poll
      ไม่ทำ heartbeat ขาด (SC-004)
- [x] T038 [US2] Restart recovery test ใน `tests/integration/restart.test.ts`:
...
- [x] T039 [P] [US3] Failure-mode tests ใน `tests/integration/failures.test.ts`:
...
- [x] T040 [US3] Login lockout ใน `src/portal/login.ts` + jobStore meta:
      นับ failure → ครบ LOGIN_MAX_RETRY → lockout LOGIN_LOCKOUT_MINUTES +
      system alert (FR-009) — ระหว่าง lockout ไม่แตะ portal
- [x] T041 [US3] Layout/locale/pagination guards ใน `src/portal/jobList.ts`:
...
- [x] T042 [US3] System alert flow ใน `src/reporting/notifier.ts` +
...
- [x] T043 [US3] Wiring ครบทุก trigger ใน `src/runtime/pollLoop.ts` +
...
- [x] T048 Final gates: `npm run lint` + `npm run typecheck` +
...
- [x] T049 [P] เขียน `README.md`: สรุป setup/รัน/ops ชี้ไปที่ quickstart +
      ตาราง SC responsibility
- [x] T051 [P] Script สรุป latency ใน `src/runtime/latencyReport.ts` +
