...
- [x] T023 [P] [US1] cold-start accept (FR-005) → consolidated ใน
      `tests/integration/xtmCycle.test.ts`: กดรับงานมาเลย์ค้างที่ยังกดได้, ข้ามที่รับแล้ว, สรุปครั้งเดียว
- [x] T024 [P] [US1] accept control → consolidated ใน `tests/unit/acceptDecision.test.ts`
      + `tests/integration/xtmCycle.test.ts`: `ACCEPT_ENABLED=0` → ไม่กด; caps
      (MAX_WORDS/PER_CYCLE) → Skipped + แจ้ง (FR-012/025)
...
- [x] T029 [US1] accept orchestration ใน `src/runtime/xtmPollCycle.ts`: detect→[eligible
...
- [x] T030 [US1] cold-start accept (FR-005) ใน `src/runtime/{xtmPollCycle,bootstrap}.ts`:
      baseline + กดรับงานค้างที่ยังกดได้ (fallback baseline-only ถ้าแยกไม่ได้)
- [x] T031 [US1] kill-switch + caps ใน xtmPollCycle: `ACCEPT_ENABLED`, `ACCEPT_MAX_WORDS`,
...
- [x] T034 [P] [US2] ensureHeader → consolidated ใน `tests/unit/sheets.test.ts`:
      v1→v2 เติม I–M ไม่ย้ายของเดิม, header แปลก→fail loud ไม่ทับ (contracts/sheets.md)
- [x] T035 [P] [US2] sheets lifecycle → consolidated ใน `tests/integration/xtmCycle.test.ts`
      + `tests/unit/sheets.test.ts`: New→Accepted/Missing/Skipped/Accept failed→Closed/
      Removed sync ลงชีต — **ใช้ fixture สถานะ ไม่เรียก accept จริง (N6)**
- [x] T036 [P] [US2] closed-check → consolidated ใน `tests/integration/xtmCycle.test.ts`:
      accepted หายจาก Active → เช็ค Closed → `Closed` (เจอ)/`Removed` (ไม่เจอ) (FR-014)
- [x] T037 [P] [US2] historical-rows → consolidated ใน `tests/unit/sheets.test.ts`:
      แถวเดิมไม่มี job_key = historical → ไม่ claim/ไม่ update (FR-026)
...
- [x] T041 [US2] คำนวณ `lifecycle_status` + enqueue outbox sheets/chat ตอน transition
      ใน `src/runtime/xtmPollCycle.ts` + `src/state/jobStore.ts` (ไม่ใส่ logic นี้ใน diff)
- [x] T042 [US2] Closed-tab targeted check → Closed/Removed (FR-014) ใน xtmPollCycle ใช้
...
- [x] T045 [P] [US3] cold-start summary → consolidated ใน `tests/unit/xtmNotifier.test.ts`
      + `tests/integration/xtmCycle.test.ts`: 1 ข้อความสรุปงานค้างตอน start (FR-005)

### Implementation for US3

- [x] T046 [US3] เพิ่ม template ข้อความใหม่ใน `src/reporting/notifier.ts` (new/
      accepted/failed) — reuse formatter เดิม
- [x] T047 [US3] cold-start summary message + wire ใน bootstrap/xtmPollCycle
- [x] T048 [US3] wire เหตุการณ์ (new/accepted/failed) → outbox channel `chat` ใน
      `src/runtime/xtmPollCycle.ts` (1 ข้อความ/งาน ไม่ batch)
...
- [x] T053 [P] [ops] อัปเดต `.env` จริงจาก partner→XTM + แชร์ชีตให้ service account
...
- [x] T055 [P] อัปเดต CLAUDE.md สถานะ 002 + docs; redeploy PM2 single instance
      ...
      **เสร็จ**: CLAUDE.md อัปเดต+commit, PM2 acolad-bot redeploy (single instance)
      + pm2 save, heartbeat ok ทุก cycle
...
- **US3 (P3)**: หลัง Foundational — ฟัง event จาก xtmPollCycle; ทดสอบ formatter แยกได้

### Within Each User Story

- เขียน test ก่อน + ต้อง FAIL → แล้ว implement (Constitution II)
- pure (eligibility) ก่อน adapter (xtmAccept) ก่อน orchestration (xtmPollCycle)
...
- **ACCEPT_ENABLED = 0 (operational)**: เปิดทดลองเป็น 1 แล้ว **ปิดกลับเป็น 0** หลัง round-2
      review ชี้ว่า `acceptAvailable` hard-code `true` (D6) ทำให้ determineAcceptOutcomes
      ได้แค่ failed/missing **ไม่มีทางเป็น accepted** — แม้กดรับสำเร็จก็ติดป้ายผิด + spam
      accept_failed alert. ต้อง recon จับสัญญาณ accept สด (Accept task vs Finish task,
      D4/D6) → คำนวณ `acceptAvailable` จริงใน `src/portal/xtmInbox.ts` ก่อนเปิด 1 อีกครั้ง.
      ตอนนี้บอททำงานโหมด detect+log+notify เต็มรูปแบบ
