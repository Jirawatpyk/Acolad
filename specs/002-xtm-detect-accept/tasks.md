...
- **ACCEPT_ENABLED**: ตั้งต้น = 0; **เปิดเป็น 1 แล้ว (operational, ผู้ใช้สั่ง)** พร้อม
      `ACCEPT_MAX_PER_CYCLE=1` เป็นกันชนรอบแรก. ⚠️ accept flow **ยังไม่ verify live** —
      `acceptAvailable` hard-code `true` (D6) + เมนู accept จาก screenshot (D4 ยังไม่จับ DOM
      จริง). fail-loud คุ้มกัน (ไม่กดมั่ว เก็บ evidence) แต่ต้องยืนยัน selector ตอนงานมาเลย์
      ตัวแรก (capture เมนูจริง → แก้ `src/portal/selectors.ts` XTM.accept ถ้าต่าง)
