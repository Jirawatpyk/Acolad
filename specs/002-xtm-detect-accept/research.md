...
### D4 อัปเดต — เมนู Accept (จาก screenshot ผู้ใช้ + capture รอบที่ 4)

เปิด kebab ของแถว (`button[data-testid="context-menu-button"]`) → **Radix dropdown
render ใน `#context-menus-container` (ใน iframe)**. รายการ submenu parent **เปลี่ยน
ข้อความตาม state ของงาน**:
- งาน **รับได้** → **"Accept task"** → submenu 6 ตัวเลือก (จาก screenshot ผู้ใช้):
  1. Accept this task
  2. Accept this task for all steps of this language
  3. Accept all tasks for this step of this language in this group
  4. **Accept all tasks for this language in this group** ← bulk Malay (FR-006) ✅
  5. Accept all tasks for this step in this group
  6. Accept all tasks in this project in this group
- งาน **in-progress/เสร็จ 100%** → **"Finish task"** (ไม่ใช่ "Accept task") = ตัวบ่งชี้
  ว่า **ไม่ใช่งานที่รับได้** (ใช้แยก acceptAvailable: มี "Accept task" = รับได้)

**ยังต้องเก็บ evidence-first** (ตอน recon มีแต่งานเสร็จ 100% → เจอ "Finish task"):
DOM เป๊ะของ submenu "Accept task" + **สัญญาณสำเร็จหลังกด** → T026 จึงยืนยันผลด้วย
**re-read Active (FR-024)** ไม่เชื่อการคลิก; ข้อความเมนูเป็น locale-dependent → fail
loud ถ้าไม่เจอบนงานที่ eligible. `ACCEPT_ENABLED=0` จนกว่าจะ capture flow จริง
