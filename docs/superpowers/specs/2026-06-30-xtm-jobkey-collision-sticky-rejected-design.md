...
**Closed-grid drift guard (extend).** `readClosedKeys`' existing detector (`xtmInbox.ts:364`, fires on `candidateCount>0 && allStepRoleNull`) does not watch project. After Fix 1 a blank/missing Closed project cell would make every key start `|file|step|role` and never match Active → all finished jobs misclassified. Extend the detector to also WARN when candidate rows exist but **every** row reads project = null — a new `allProjectNull` flag, symmetric with the existing `allStepRoleNull`. Keep the per-row skip on `!r.file` **only**; do **not** add an empty-project `continue` — it would skip project-null rows before they are counted, making the new `allProjectNull` WARN impossible to reach (self-defeating). An empty-project key `|file|step|role` cannot false-match the Active side (which always carries a real project), so it is not dangerous the way an empty file is.
...
**Migration — targeted backfill of non-terminal rows.** "New key forward, no backfill" is **unsafe**: the bot auto-accepts Malay jobs 24/7 and a job accepted-but-not-yet-finished stays **held** in Active (`lifecycleStatus 'accepted'`). At the next `npm run deploy` restart, held/`accepting` rows under the OLD key would (a) mis-disappear → be misclassified **Removed** (Accepted record lost, quota released), and (b) reappear under the NEW key → a fake **New Job** + a re-accept attempt on a job we already own (fake "Accepted" card, new `acceptedAt`, ~2-cycle capacity double-count) — **the same failure mode as 4721900**. Therefore the migration **re-keys every NON-TERMINAL row** to the new composite, computed deterministically from the row's stored `project_name|file_name|step|role`. The predicate must use the **real enum values** (`src/detection/types.ts:75-83`, `db.ts:147` = `new, accepted, skipped, missing, accept_failed, closed, removed, rejected`): **`lifecycle_status IN ('new','accepted','skipped','accept_failed','rejected') OR accept_status IN ('accepting','accepted')`**. (`present`/`accepting` are **not** `lifecycle_status` values — `accepting` belongs to `accept_status`, already covered by the OR; the prior draft's `NOT IN ('closed','missing','removed')` is wrong because legacy feature-001 rows can have `lifecycle_status = NULL` and would be pulled in.) Terminal rows (closed/missing/removed) keep their old keys (never re-read). The held job's pre-existing Sheet row (old key in col N) remains and the bot appends one fresh row under the new key on the next upsert — a **permanent phantom "Accepted" row** per held job (the old-key row never resolves to Closed, since the job is now tracked under the new key); acceptable because capacity + the daily report read the DB lifecycle, not the Sheet. The backfill runs once, inside the same DB migration that adds `reject_reason` (idempotent: a row whose key already equals its new composite is a no-op).

---

## 3. Fix 2 — sticky Rejected Sheet status

1. **Persist the reason.** Add `rejectReason: string | null` to `XtmJobState` and a `reject_reason TEXT` column to the jobs table — add `{ name: 'reject_reason', ddl: 'reject_reason TEXT' }` to `JOB_V2_COLUMNS` (`db.ts:132`) so `ensureJobColumns` adds it idempotently and `widenLifecycleCheck`'s rebuild carries it via `allColumns` (`db.ts:205`). It is a **business field, not from-raw** (unlike `file_wwc`): `buildXtmState` (`xtmDiff.ts:53`) initialises `rejectReason: null`; `applyXtmState` (`:87`) preserves it via `...existing` (there is nothing to read from the raw snapshot). **Critically, the store write path must be edited explicitly:** `XtmJobStore.upsertMany` (`xtmJobStore.ts:102-151`) is a hand-written `INSERT … VALUES … ON CONFLICT DO UPDATE SET` with a bound params object that is **not** type-checked — its column list, `VALUES`, the `ON CONFLICT … SET`, and the params object must all add `reject_reason`, and `rowToState` must read `r.reject_reason`. (`rowToState`/`XtmJobRow` are compiler-forced; `upsertMany` is not — omitting it makes `reject_reason` silently never persist → Fix 2 breaks across restart.)

2. **Precise set/clear** (the cycle owns this; `blockNotes` holds skip reasons too, so do **not** set from it blindly): at the start of handling each **present, evaluated** job — before `decideAccept` — set `s.rejectReason = null`. There are **two** `decideAccept` sites and both must clear: the **event pass** (`xtmPollCycle.ts:281`, first_seen/relisted/cold-start) and the **robustness pass** (`:333`, present jobs with no event). In the **event pass the clear MUST sit after the `if (ev.eventType === 'missing')` block** (`:266-278`) — never at the eligibility loop (`:177`, which iterates all `nextStates` including the missing job) nor before the missing branch, or a rejected job leaving Active (Fix 2's core case) would have its reason cleared before reaching the helper and render an empty "Missing". Then **only** the gate-reject branch (`xtmPollCycle.ts:~427-429`, `lifecycleStatus='rejected'`) sets `s.rejectReason = note` (the same `group blocked: ${blockReason}` string it builds for the Sheet/Chat). Every other present path (first_seen/relisted pre-decide, `skipped`, `ACCEPT_ENABLED`-off `new`, kill-switch candidate, accepted) thus leaves it null. An **absent** (missing/closed/removed) job is not in the snapshot and is not re-evaluated, so its last value **persists** — this is what makes a rejected job's reason survive after it leaves Active. (Clearing transiently before the gate re-sets is safe: nothing reads `rejectReason` between the clear and the gate-set; the persist `upsertMany` and the Sheet enqueue both run later.)
