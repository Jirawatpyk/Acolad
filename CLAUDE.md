...
- **claim = `POST /api/vendors/{vendorId}/job-offers/{obj_id}/accept`** (ยืนยันจาก JS ของเว็บ
  พอร์ทัลเอง 2026-09-18). เดิมเดาเป็น `/claim` → claim จริงครั้งแรก (`4b7be68e`, 18/09 12:22)
  ได้ `404 Not Found` ทั้งที่งานยังเปิดอยู่อีก 2 นาที = route ไม่มีอยู่จริง ไม่ใช่แพ้.
  **แพ้การแข่ง = HTTP 409 เท่านั้น** (`CONFIRMED_LOST_RACE_SIGNALS = ['http_409']` — เว็บพอร์ทัล
  แสดง "Offer no longer available" เฉพาะ 409) → `lost` ไม่ alert. รหัสอื่นทุกตัว (รวม 404) ยังเป็น
  `failed` + alert (FR-005a). **RP-4 ปิดแล้ว (2026-09-22)**: ชนะจริงผ่าน `/accept` แล้ว 17 ครั้ง; ยังไม่เคยเห็น
  409 จริง — `lost` บรรทัดแรกใน log คือการยืนยันสุดท้าย ไม่ต้องแก้โค้ด
...
- **`due_at` ของ offer มีเศษวินาที 6 หลักได้ — เคยทำให้บอทเมินงานทั้งเช้า** (แก้แล้ว 2026-09-25):
  พอร์ทัลส่ง `2026-09-28T00:05:50.987723` มา แต่ `offerParse` รับเฉพาะ `YYYY-MM-DDTHH:mm:ss`
  เป๊ะ ๆ → offer 6 ใบของวันที่ 25/09 ถูกทิ้งเป็น `unreadable` ทั้งหมด (846 บรรทัด parse fail,
  ไม่มีการตัดสินใจสักใบ) ตอนนี้ parser รับเศษแล้วและ**ตัดเหลือ 3 หลัก** (ECMAScript รับแค่ 3 —
  ป้อน 6 หลักคือพึ่ง engine) ส่วน**กฎห้ามมีโซนยังเข้มเหมือนเดิม** เพราะเศษวินาทีไม่ได้บอกโซน
  - DL แบบนี้เป็น **เวลาที่เครื่องคำนวณให้** (เวลาสร้าง + 72 ชม.) ต่างจาก DL ที่คนตั้งซึ่งลงท้าย
    กลม ๆ เช่น `08:00:00` — นี่คือเหตุผลที่ offer ก่อนหน้าไม่เคยเจอ
  - `reconcile.ts` (`ASSIGNED_DUE_AT`) รับเศษวินาทีมาตั้งแต่แรกแล้ว มีแต่ฝั่ง offer ที่ตกหล่น
  - **ระบบเตือนทำงานถูกต้อง**: alert `offer_unreadable` ออก 6 ใบ ใบละ offer ตามดีไซน์ — ถ้าเห็น
    การ์ดชุดนี้อีก ให้ดู field ใน log `module:offerParse action:parse outcome:unreadable` ว่าคราวนี้
    รูปแบบไหนเปลี่ยน อย่าเดาแล้วผ่อนกฎ
...
- **PO ที่ยังเป็น `AWAITING ACCEPTANCE` (pending) ไม่มีภาษาและไม่มี DL — ได้มาตอนถูก accept**
  (ยืนยันจากหน้าพอร์ทัลทั้ง 43 PO เมื่อ 2026-09-23) คีย์ของมันจึงเป็น `job_ref||service` ซึ่งบอกแค่
  *งานอ้างอิงไหน บริการอะไร* ไม่บอกว่า*งานชิ้นไหน* ("bucket" ไม่ใช่ identity)
  - **ทุกงานที่ชนะผ่านสถานะนี้เสมอ** ไม่ใช่เรื่องเฉพาะงาน per-hour ตามที่เข้าใจตอนแรก ปกติคนกด
    accept ภายในไม่กี่นาที reconcile (ทุก 15 นาที) จึงมักเห็นตอนที่มีภาษาแล้ว — นี่คือเหตุผลที่
    PO ชุด +7 ของคืนวันที่ 22 จับคู่ได้ปกติ (ถูก accept ไปก่อน reconcile จะอ่าน)
  - เอกสารเดิมเขียนว่าภาษาว่าง = งาน DTP ซึ่งผิดสองชั้น: **PO ของ DTP มีภาษา** (`Japanese →
    Japanese` — source เท่ากับ target ซึ่ง `workKey` ยุบให้เอง) ส่วนที่ว่างจริงคือ PO ที่ยัง pending
  - เกิดจริงกับ `aj-345` (งาน per-hour ที่ไม่มีใคร accept นาน 3 ชม.): ชนะ 7 คู่ภาษา แล้ว PO 6 ใบ
    คีย์ `aj-345||translation` จับคู่ไม่ติด → ถูก recover เป็นงานใหม่ทั้งหมด ได้ 13 แถวสำหรับ
    งาน 7 ชิ้น + การ์ดซ้ำ 6 ใบ
  ...
  - log warn `module:reconcile action:order_languageless` พร้อม `outcome:covered|recovered`
    ...
    หนึ่งงานมี PO หลายใบ จะกลายเป็นการ์ดซ้ำชุดใหม่แทนชุดเก่า). **`outcome:covered` เป็นเรื่องปกติ
    ไม่ใช่สัญญาณเตือน** — จะเห็นทุกรอบจนกว่าจะมีคนกด accept PO นั้น
  ...
  - ล้างแถวซ้ำที่ค้างใน DB: `npm run straker:drop-phantom-holds` (dry-run, ต้อง `--apply` ถึงเขียน)
    สคริปต์แยก "แถวผี" ออกจาก **งาน monolingual จริง** ที่คีย์เป็น `ref||service` โดยชอบธรรมด้วย
    การดูว่ามี `recovery` event ไหม (แถวผีมี งานที่บอทกดเองไม่มี) และปล่อยไม่เกินจำนวนแถวพี่น้อง
    ที่คีย์มีภาษา — ส่วนเกินรายงานไว้ให้คนดู ไม่ปล่อย
  - ⚠️ **ช่วงระหว่าง deploy กับรันสคริปต์ล้าง จะมีการ์ด 🔴 `held_work_undated` ออกมา 1 ใบต่อแถวผี**
    (ตอนนี้ 6 ใบ) เพราะ reconcile รันตั้งแต่ตอน start ก่อน poll รอบแรก แต่ละใบบอกให้ไปหา DL ของแถว
    ที่สคริปต์กำลังจะลบ — **คาดไว้แล้ว ไม่ใช่ของเสีย** ส่วนแถวจริง 7 แถวเข้าเส้นทาง "ถือไว้" ในช่วงนี้
    จึงไม่มีอะไรจริงถูกปล่อย
...
- **alert อีก 2 ตัว (2026-09-23)** ครั้งเดียวต่อ offer เหมือนกัน:
  ...
  เปิดอยู่ → เช็กบนพอร์ทัลว่า order ยังเปิดจริงไหม (PO ที่ยังไม่ถูก accept เป็นแบบนี้ปกติ)
...
ข้อสรุปพร้อมชื่อ (RP-4 ปิดแล้ว 2026-09-22). อยู่ใน
