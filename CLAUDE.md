...
> `ACCEPT_EFFORT_METRIC` คุม *metric วัด effort* (`wwc` ค่า default — cap/throughput
> ใช้ File WWC หลังหัก TM, ต้องตั้ง `ACCEPT_MAX_WWC_PER_DAY` ≈ 1000;
> `words` = พฤติกรรมเดิม byte-for-byte). live = ทั้งสาม `=1`/`=1`/`=wwc`.
