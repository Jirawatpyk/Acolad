...
import { LayoutChangedError, PaginationDetectedError } from './errors.js';
...
export async function readJobSnapshot(
  ...
  // Allow the SPA to render the offers shell after the post-login redirect.
  await container.waitFor({ state: 'attached', timeout: 10_000 }).catch(() => undefined);
  ...
  // Wait for the offers content to render: either offer rows or the empty-state
  // marker. The nav shell (container) appears before the list body, so without
  // this we'd read an in-between state and wrongly fail loud.
  await page
  ...
  // explicit empty-state marker. This is ambiguous — likely an unknown offer-row
  // structure — so capture evidence and treat as a layout change rather than
